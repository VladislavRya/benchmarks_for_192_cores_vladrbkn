...
TPM_FILE="tmp_file.txt"

# Create results directory
mkdir -p results

# Make twice because single sometimes does not work
make bt-mz CLASS=C
make bt-mz CLASS=C

# Results file
RESULTS="results/benchmark_results.txt"
echo "OpenMP NBP BT-MZ Benchmark Results" > $RESULTS
echo "=========================" >> $RESULTS
...
THREADS=(1 24 48 72 96 120 144 168 192)
NUM_RUNS=3
    ...
    for xstreams in "${XSTREAMS[@]}"; do
        for threads in "${THREADS[@]}"; do
            ...
            for run in $(seq 1 $NUM_RUNS); do
                ...
                OUTPUT_FILE="results/bt-mz_${class}_x${xstreams}_t${threads}_run${run}.txt"
                
                OMP_NUM_THREADS=$xstreams OMP_NUM_THREADS2=$threads timeout 750 /usr/bin/time -o $TPM_FILE -f "%e" bin/bt-mz.${class}.x > $OUTPUT_FILE
                
                # Extract execution time and verification status
                TIME=$(grep "Time in seconds" $OUTPUT_FILE | awk '{print $NF}')
                TIME_NANOS=$(cat $TPM_FILE)
